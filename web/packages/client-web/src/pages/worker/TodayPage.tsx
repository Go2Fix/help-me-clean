...
import { TODAYS_JOBS } from '@/graphql/operations';
...
interface Job {
  ...
  status: string;
  address: { streetAddress: string; city: string; floor?: string; apartment?: string } | null;
...
export default function TodayPage() {
  ...
  const { data, loading } = useQuery(TODAYS_JOBS);
  const jobs: Job[] = data?.todaysJobs ?? [];

  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">Comenzile de azi</h1>
        <p className="text-gray-500 mt-1">
          {jobs.length > 0
            ? `${jobs.length} ${jobs.length === 1 ? 'comanda programata' : 'comenzi programate'}`
            : 'Nicio comanda programata'}
        </p>
      </div>
