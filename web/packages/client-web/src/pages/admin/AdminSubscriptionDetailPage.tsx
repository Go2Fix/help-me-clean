...
export default function AdminSubscriptionDetailPage() {
  ...
  const [changeWorker, { loading: changingWorker }] = useMutation(REQUEST_SUBSCRIPTION_WORKER_CHANGE, {
    refetchQueries: [{ query: SUBSCRIPTION_DETAIL, variables: { id } }],
    onCompleted: () => {
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-600">
            Sistemul va aloca automat cel mai potrivit lucrator disponibil pentru sesiunile viitoare.
          </p>
          {workerChangeError && (
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <Button loading={changingWorker} onClick={() => changeWorker({ variables: { id: sub.id } })}>
              Confirma schimbarea
            </Button>
          </div>
        </div>
