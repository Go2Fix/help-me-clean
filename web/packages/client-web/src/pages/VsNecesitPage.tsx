...
export default function VsNecesitPage() {
  const { t } = useTranslation('vs');
  const { lang } = useLanguage();
  const { isPreRelease } = usePlatform();

  const rows = t('necesit.features.rows', { returnObjects: true }) as ComparisonRow[];
  const reasons = t('necesit.reasons', { returnObjects: true }) as Reason[];

  return (
    <>
      <SEOHead
        title={t('necesit.meta.title')}
        description={t('necesit.meta.description')}
        canonicalUrl={ROUTE_MAP.vsNecesit[lang]}
        lang={lang}
        alternateUrl={{ ro: ROUTE_MAP.vsNecesit.ro, en: ROUTE_MAP.vsNecesit.en }}
        structuredData={{
          '@context': 'https://schema.org',
          '@type': 'WebPage',
          name: t('necesit.hero.title'),
          description: t('necesit.meta.description'),
          url: `https://go2fix.ro${ROUTE_MAP.vsNecesit[lang]}`,
        }}
      />
      ...
      <section className="bg-gradient-to-b from-blue-50 to-white py-16 px-4">
        <div className="max-w-4xl mx-auto text-center">
          <span className="inline-block text-xs font-semibold uppercase tracking-widest text-blue-600 bg-blue-100 rounded-full px-3 py-1 mb-4">
            {t('necesit.hero.badge')}
          </span>
          <h1 className="text-4xl sm:text-5xl font-bold text-gray-900 mb-4">
            {t('necesit.hero.title')}
          </h1>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            {t('necesit.hero.subtitle')}
          </p>
        </div>
      </section>
      ...
      <section className="py-16 px-4">
        <div className="max-w-xl mx-auto text-center">
          ...
          <p className="text-gray-500 mb-8">{t('necesit.ctaSubtitle')}</p>
          {isPreRelease ? (
            <Link
